...
<h1>Assignment: Automobiles and Databases</h1>
<p>
In this assignment you will build a web based application to track data about automobiles and store the data in a MySQL database.</p>
...
<pre><a href="https://www.wa4e.com/solutions/autosdb/" target="_blank">http://www.wa4e.com/solutions/autosdb/</a>
</pre>
<h1>Resources</h1>
...
<a href="https://php.net/manual/en/book.pdo.php" target="_blank">PDO</a> to connect to a database.
</li>
<li>Documentation on 
<a href="https://en.wikipedia.org/wiki/SQL_injection" target="_new">SQL Injection</a>
</li>
<li>Documentation on 
<a href="https://php.net/manual/en/pdo.prepare.php" target="_new">PHP PDO Prepared Statements</a>
</li>
<li>
You can look through the sample code from the lecture. It has examples
of using PDO to communicate with a database:
<pre><a href="https://www.wa4e.com/code/pdo.zip" target="_blank">http://www.wa4e.com/code/pdo.zip</a>
</pre>
<p>
...
<pre style="color:red">Email and password are required
</pre>
...
<pre style="color:red">Email must have an at-sign (@)
</pre>
</p>
<p>
If the incoming password, properly hashed matches the stored stored_hash
value, the user's browser is
<a href="https://en.wikipedia.org/wiki/URL_redirection#Using_server-side_scripting_for_redirection" target="_blank">redirected</a>
to the <b>autos.php</b> page with the user's name as a GET parameter using:
<pre>header("Location: autos.php?name=".urlencode($_POST['who']));
</pre>
</p>
<p>
...
plus the salt:
<pre>error_log("Login fail ".$_POST['who']." $check");
</pre>
When the login succeeds (i.e. the hash matches) issue the following log message:
<pre>error_log("Login success ".$_POST['who']);
</pre>
Make sure to find your error log and find those error messages as they come out:
<pre>[11-Feb-2016 15:52:03 Europe/Berlin] Login success <a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="47243422310726323328346924282a">[email&#160;protected]</a>
[11-Feb-2016 15:52:13 Europe/Berlin] Login fail <a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="ee8d9d8b98ae8f9b9a819dc08d8183">[email&#160;protected]</a> 047398bd0e0171f4954760f5f542121a
</pre>
...
<pre>die("Name parameter missing");
</pre>
...
<pre style="color:green">Record inserted
</pre>
</p>
<p>
...
<ol>
<li>A screen shot (including the URL) of your login.php rejecting an account without an at-sign (@)</li>
<li>A screen shot of your error log showing correct messages for both a successful and failed login attempt.</li>
<li>A screen shot (with URL) of your autos.php showing 'Name parameter missing'</li>
<li>A screen shot (including the URL) of your autos.php with three vehicles in the list.  At least one of the vehicles must have '&lt;b&gt;' in its title and it must be shown properly (i.e. the title should not be bold)</li>
<li>A screen shot (including the URL) of your autos.php showing the error message for a non-numeric year</li>
<li>A screen shot of your autos database table in a database tool showing at least three vehicles</li>
<li>Source code of login.php</li>
<li>Source code of autos.php</li>
</ol>
</p>
<h2>Grading</h2>
<p>
Don't take off points for little mistakes.  If they seem to have done the assignment give them full credit.   Feel free to make suggestions if there are small mistakes.  Please keep your comments positive and useful.  If you do not take grading seriously, the instructors may delete your response and you will lose points.</p>
<p>
...
<pre>&lt;a href="http://....jpg" target="_blank"&gt;Ford&lt;/a&gt;
</pre>
</li>
<li>
