    ...
    if (
        ...
        try{
            ...
            for($i=1; $i<=9; $i++) {
                ...
                $stmt = $pdo->query("SELECT `institution_id` from `institution` WHERE name = \"$edu_school\"");
                $row = $stmt->fetch(PDO::FETCH_ASSOC);
                $edu_school = $row['institution_id'];

                $stmt = $pdo->prepare('INSERT INTO `education`(`profile_id`, `institution_id`, `rank`, `year`) VALUES (:pid ,:Inid,:rank ,:year )');
